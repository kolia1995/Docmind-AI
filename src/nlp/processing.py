...
class FileTextExtractor:
    ...
    def read(self):
        if not self.file and not self.url:
            logger.warning("No file or URL provided for extraction")
            return False

        if self.url:
            logger.info(f"Fetching content from URL: {self.url}")
            if self._is_blocked_host():
                logger.warning(f"URL blocked: {self.url}")
                return False
            self._fetch_url()

        if not self.file:
            logger.warning("File is None after processing")
            return False
        ...
        if self._is_blocked_file(ext):
            logger.warning(f"File is blocked or invalid: {self.file.filename}")
            return False
        ...
        if not reader:
            logger.error(f"No reader found for extension: {ext}")
            return False

        logger.debug(f"Reading {ext} file")
        reader()
        ...
        logger.info(f"Text extraction completed (length: {len(cleaned_text)})")
        return cleaned_text
